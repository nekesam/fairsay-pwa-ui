...
export default function Dashboard() {
  ...
  const [learningData, setLearningData] = useState({
    ...
    completedCount: 0,
    totalReq: courses.length,
    overallProgress: 0
  });

  // Sync Activities & AI Count
  useEffect(() => {
    if (user?.id) {
      try {
  ...
  // Fetch True Backend Stats (Complaints + Learning)
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Fetch True Learning State (Cache-Busted!)
        ...
        // Dynamically calculate actual module progress
        ...
        const totalReq = enriched.length;
        const overallProgress = Math.round((completedCount / totalReq) * 100) || 0;

        setLearningData({ enrichedCourses: enriched, completedCount, totalReq, overallProgress });
      ...
      <ActionBanners />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12 px-2">
          <TopStat 
            ...
            value={`${learningData.overallProgress}%`} 
            detail={`${learningData.completedCount} of ${learningData.totalReq} modules`} 
            trend={learningData.overallProgress === 100 ? "Completed" : learningData.overallProgress > 0 ? "In Progress" : "Not Started"} 
            corner="bg-[#0F766E]/20" 
            iconBg="bg-[#0F766E]" 
          />
          <TopStat 
          ...
          />
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                {/* Dynamically locked using real DB stats */}
                <ActionCard 
                  icon={report} 
...
<div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-10 bg-white p-6 rounded-xl border border-gray-100 shadow-sm max-w-full">
          ...
          <div className="lg:col-span-4 space-y-6">
            <EducationSidebar learningData={learningData} />
  ...
  return (
    <div className={`${color} p-6 rounded-2xl text-white relative flex flex-col justify-between min-h-[200px] gap-4 mb-[20px]`}>
      ...
      <button 
        onClick={onClick} 
        disabled={locked}
        className={`bg-white/10 hover:bg-white/20 transition-colors h-max-[48px] font-inter py-2.5 w-fit px-5 rounded-xl text-[16px] font-bold flex items-center justify-center gap-2 ${locked ? 'opacity-50 cursor-not-allowed hover:bg-white/10' : ''}`}
      >
        {btnText} <span>→</span>
      </button>
    </div>
...
function EducationSidebar({ learningData }) {
  const navigate = useNavigate();
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-300 p-6 shadow-lg">
      ...
      <div className="space-y-5 mb-8">
        {learningData.enrichedCourses.map((m) => {
          
          const progress = m.actualProgress;
          const isDone = progress === 100;
          
          return (
            <div key={m.id} className={!m.isUnlocked ? "opacity-50 grayscale" : ""}>
      ...
      >
        {learningData.overallProgress === 100 ? "Review Modules" : "Continue Learning"} <span>→</span>
      </button>
    </div>
  );
}
