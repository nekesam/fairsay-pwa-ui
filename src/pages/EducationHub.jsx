...
import { getProgress, getProgressStats } from "../utils/courseProgress";
...
function CourseCard({ course, progress }) {
  const courseProgress = progress[course.id] || { unlocked: false, completed: false, score: 0 };
  const isUnlocked = courseProgress.unlocked === true;
  const isCompleted = courseProgress.completed === true;
  const progressPercent = isCompleted ? 100 : 0;
  const lessonsCompleted = isCompleted ? course.lessons.length : 0;
  
  return (
    <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all ${!isUnlocked ? 'opacity-60' : ''}`}>
      <div className="p-6">
        {/* Header */}
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-gray-500 font-medium">
              {isCompleted ? 'Completed' : isUnlocked ? 'Progress' : 'Locked'}
            </span>
            <span className="font-bold" style={{ color: course.color }}>
              {isCompleted && courseProgress.score > 0 ? `${courseProgress.score}% score` : `${progressPercent}%`}
            </span>
          </div>
